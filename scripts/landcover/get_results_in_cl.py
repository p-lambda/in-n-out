...
if __name__ == "__main__":
    for exp_name in ['baseline', 'aux-inputs', 'aux-outputs']:
        ...
        cmd = f'cl run -n landcover_{exp_name} -w {WKSHT_NAME} --request-docker-image ananya/in-n-out --request-queue tag=nlp {deps} :get_pastable.py "python get_pastable.py -a {dirnames}"'
    ...
    for it in [0, 1]:
        ...
        cmd = f'cl run -n landcover_{exp_name}_iter{it} -w {WKSHT_NAME} --request-docker-image ananya/in-n-out --request-queue tag=nlp {deps} :get_pastable.py "python get_pastable.py -a {dirnames}"'
